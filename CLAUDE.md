...
Several modules read from a shared PostgreSQL instance at `10.93.143.200:5432`, user/db `gruppe3/postgres`. The connection host can be overridden with the `PG_HOST` environment variable. Modules fall back gracefully when the DB is unavailable.
